...
def kdj(
    *,
    ...
    """
    n_bars = len(close)
    if len(high) != n_bars or len(low) != n_bars:
        raise OprimError(f"Length mismatch: high({len(high)}), low({len(low)}), close({n_bars})")
    if n > n_bars:
        raise OprimError(f"Sequence length {n_bars} is less than period n={n}")
    if n < 1:
        raise OprimError(f"n must be >= 1, got {n}")
    if k_smooth < 1 or d_smooth < 1:
        raise OprimError(
            f"smooth periods must be >= 1, got k_smooth={k_smooth}, d_smooth={d_smooth}"
        )
    ...
    k_arr = np.empty(n_bars, dtype=np.float64)
    d_arr = np.empty(n_bars, dtype=np.float64)
    ...
    return KDJResult(k=k_arr.tolist(), d=d_arr.tolist(), j=j_arr.tolist())
