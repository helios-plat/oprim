...
class LimitStatusResult(BaseModel):
    ...
    recent: list[str] = Field(
        ..., description="近 lookback 日状态 ['limit_up'|'limit_down'|'normal']"
    )
...
def limit_status_calc(
    *,
    ...
    if n_bars < 2:
        ...
        if lookback > 0:
            raise OprimError(
                f"Sequence length {n_bars} insufficient to determine status for lookback {lookback}"
            )
        return LimitStatusResult(recent=[])

    if lookback > n_bars - 1:
        raise OprimError(f"lookback {lookback} exceeds available status points {n_bars - 1}")
