...
class TestOkxWsMsg:
    ...
    async def test_subscribe_message_sent(self):
        ...
        with patch("websockets.connect", return_value=mock_ws):
            await okx_ws_msg(
                "books5", inst_id="ETH-USDT-SWAP", callback=lambda _: None, timeout=0.05
            )
    ...
    async def test_connection_error_raises(self):
        ...
        with patch("websockets.connect", side_effect=OSError("refused")), pytest.raises(OkxWsError):
    ...
    async def test_error_event_raises(self):
        ...
        with patch("websockets.connect", return_value=mock_ws), pytest.raises(OkxWsError):
