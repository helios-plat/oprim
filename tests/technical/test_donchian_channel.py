...
def test_donchian_basic():
    ...
    result = donchian_channel(highs, lows=lows, window=3)
...
def test_donchian_constant_prices():
    ...
    result = donchian_channel(prices, lows=prices, window=5)
...
def test_donchian_mismatched_length_raises():
    with pytest.raises(ValueError):
        donchian_channel(np.array([1.0, 2.0, 3.0]), lows=np.array([0.5, 1.5]))


def test_donchian_invalid_window():
    with pytest.raises(ValueError):
        donchian_channel(np.arange(1.0, 10.0), lows=np.arange(0.5, 9.5), window=0)
    with pytest.raises(ValueError):
        donchian_channel(np.arange(1.0, 10.0), lows=np.arange(0.5, 9.5), window=-1)


def test_donchian_nan_before_window():
    highs = np.arange(1.0, 11.0)
    lows = np.arange(0.5, 10.5)
    result = donchian_channel(highs, lows=lows, window=4)
...
def test_donchian_matches_manual_rolling_max_min():
    ...
    result = donchian_channel(highs, lows=lows, window=window)
