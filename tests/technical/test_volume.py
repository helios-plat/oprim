...
def test_obv_increases_on_up_day():
    ...
    result = np.array(obv(closes, volumes=volumes))
    # Day 1: up → OBV[1] = 200
...
def test_obv_series_input():
    ...
    result = obv(pd.Series(c), volumes=pd.Series(v))
    assert isinstance(result, pd.Series)


def test_obv_empty_raises():
    with pytest.raises(ValueError, match="empty"):
        obv(np.array([]), volumes=np.array([]))
...
def test_mfi_output_shape():
    ...
    result = mfi(h, lows=low, closes=c, volumes=v)
    assert len(result) == 100


def test_mfi_nan_prefix():
    h, low, c, v = _make_data(100)
    result = mfi(h, lows=low, closes=c, volumes=v, period=14)
    arr = np.array(result)
...
def test_mfi_normalized_range():
    h, low, c, v = _make_data(100)
    result = mfi(h, lows=low, closes=c, volumes=v, normalize=True)
    arr = np.array(result)
...
def test_mfi_unnormalized_range():
    h, low, c, v = _make_data(100)
    result = mfi(h, lows=low, closes=c, volumes=v, normalize=False)
    arr = np.array(result)
    valid = arr[~np.isnan(arr)]
...
def test_mfi_series_input():
    ...
    result = mfi(pd.Series(h), lows=pd.Series(low), closes=pd.Series(c), volumes=pd.Series(v))
    assert isinstance(result, pd.Series)


def test_mfi_empty_raises():
    with pytest.raises(ValueError):
        mfi(np.array([]), lows=np.array([]), closes=np.array([]), volumes=np.array([]))


def test_mfi_invalid_period_raises():
    h, low, c, v = _make_data(50)
    with pytest.raises(ValueError, match="period"):
        mfi(h, lows=low, closes=c, volumes=v, period=0)


def test_mfi_length_mismatch_raises():
    h, low, c, v = _make_data(50)
    with pytest.raises(ValueError, match="same length"):
        mfi(h, lows=low[:30], closes=c, volumes=v)
...
def test_mfi_all_positive_flow():
    ...
    c = np.arange(1.0, n + 1)
    v = np.ones(n) * 1000
    result = mfi(h, lows=low, closes=c, volumes=v, period=5, normalize=True)
    arr = np.array(result)
    valid = arr[~np.isnan(arr)]
