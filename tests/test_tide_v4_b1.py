...
def test_beneish_normal():
    curr = BeneishInput(
        net_profit=10, revenue=100, total_assets=500, total_liabilities=200, operating_cash_flow=8
    )
    prior = BeneishInput(
        net_profit=8, revenue=80, total_assets=450, total_liabilities=180, operating_cash_flow=6
    )
...
def test_beneish_fraud_risk():
    ...
    curr = BeneishInput(
        net_profit=50, revenue=100, total_assets=500, total_liabilities=200, operating_cash_flow=-50
    )
    prior = BeneishInput(
        net_profit=10, revenue=100, total_assets=500, total_liabilities=200, operating_cash_flow=10
    )
    res = beneish_m_score(current=curr, prior=prior)
...
def test_beneish_division_by_zero():
    ...
    prior = BeneishInput(
        net_profit=10, revenue=0, total_assets=500, total_liabilities=200, operating_cash_flow=8
    )
    res = beneish_m_score(current=curr, prior=prior)
...
def test_beneish_m_score_threshold():
    ...
    curr = BeneishInput(
        net_profit=10, revenue=100, total_assets=1000, total_liabilities=100, operating_cash_flow=20
    )
    prior = BeneishInput(
        net_profit=10, revenue=110, total_assets=1000, total_liabilities=100, operating_cash_flow=20
    )
    res = beneish_m_score(current=curr, prior=prior)
    assert res.m_score < -2.22
...
def test_dcf_normal():
    ...
    res = dcf_valuation(
        free_cash_flows=fcf, discount_rate=0.10, terminal_growth_rate=0.02, shares_outstanding=1000
    )
