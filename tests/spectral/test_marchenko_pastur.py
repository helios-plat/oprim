...
class TestMarchenkoPasturThreshold:
    def test_returns_all_keys(self):
        result = marchenko_pastur_threshold(100, n_features=50)
    ...
    def test_lambda_min_zero_when_q_equals_one(self):
        # q=1: lambda_min = 0
        result = marchenko_pastur_threshold(100, n_features=100)
    ...
    def test_mass_zero_q_greater_than_one(self):
        ...
        result = marchenko_pastur_threshold(100, n_features=200)
        assert math.isclose(result["mass_zero"], 0.5, rel_tol=1e-9)

    def test_mass_zero_zero_when_q_less_than_one(self):
        result = marchenko_pastur_threshold(200, n_features=100)
        assert result["mass_zero"] == 0.0

    def test_sigma_sq_scaling(self):
        r1 = marchenko_pastur_threshold(100, n_features=50, sigma_sq=1.0)
        r2 = marchenko_pastur_threshold(100, n_features=50, sigma_sq=4.0)
    ...
    def test_invalid_n_samples(self):
        with pytest.raises(ValueError, match="n_samples"):
            marchenko_pastur_threshold(0, n_features=10)

    def test_invalid_n_features(self):
        with pytest.raises(ValueError, match="n_features"):
            marchenko_pastur_threshold(10, n_features=0)

    def test_invalid_sigma_sq(self):
        with pytest.raises(ValueError, match="sigma_sq"):
            marchenko_pastur_threshold(100, n_features=50, sigma_sq=-1.0)
