...
async def test_fetch_order_book_depth():
    r = await fetch_order_book_depth(
        client=MockHttp({"bids": [], "asks": []}), exchange="binance", symbol="BTC"
    )
...
async def test_compute_slippage():
    r = await compute_slippage_estimate(
        order_size=5, bids=[], asks=[[100, 10], [101, 10]], side="buy"
    )
...
async def test_compute_vwap():
    r = await compute_volume_weighted_price(
        bars=[{"close": 100, "volume": 10}, {"close": 200, "volume": 10}]
    )
    assert r == 150.0
...
async def test_compute_microstructure():
    r = await compute_microstructure_features(
        ticks=[{"price": 100, "qty": 1, "side": "buy"}, {"price": 101, "qty": 2, "side": "sell"}]
    )
...
async def test_cross_exchange_funding():
    ...
    r = await cross_exchange_funding_diff(
        client=MultiHttp(), symbol="BTC-USDT", exchanges=["a", "b"]
    )
    assert r["diff"] == 0.0
...
def test_benchmark_metrics():
    r = compute_benchmark_metrics(
        strategy_returns=[0.01, 0.02, -0.01], benchmark_returns=[0.005, 0.01, -0.005]
    )
...
def test_mcmc_sample():
    r = compute_mcmc_sample(
        log_posterior_fn=lambda x: -sum(xi**2 for xi in x), initial=[0.0], n_samples=50
    )
    assert len(r) == 50
