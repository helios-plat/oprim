...
def test_svensson_mismatched_lengths():
    with pytest.raises(ValueError):
        svensson_yield_curve(np.array([1.0, 2.0]), yields=np.array([0.02]))


def test_svensson_too_few_points():
    with pytest.raises(ValueError):
        svensson_yield_curve(np.array([1.0]), yields=np.array([0.02]))
...
def test_cubic_spline_clamped():
    ...
    result = cubic_spline_yield_curve(t, yields=y, boundary_type="clamped")
