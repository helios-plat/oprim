...
def test_lsm_laguerre_basis():
    result = lsm_american_price(100.0, 100.0, 1.0, 0.05, 0.20, basis_functions="laguerre", seed=0)
    assert result["price"] > 0


def test_lsm_hermite_basis():
    result = lsm_american_price(100.0, 100.0, 1.0, 0.05, 0.20, basis_functions="hermite", seed=0)
    assert result["price"] > 0
...
def test_lsm_n_basis_4():
    result = lsm_american_price(100.0, 100.0, 1.0, 0.05, 0.20, n_basis=4, seed=0)
    assert result["price"] > 0
...
def test_barrier_up_and_in_call():
    result = barrier_option_price(
        100.0, 100.0, 110.0, 1.0, 0.05, 0.20, barrier_type="up_and_in", option_type="call"
    )
    assert result["price"] >= 0
...
def test_barrier_down_and_in_put():
    result = barrier_option_price(
        100.0, 100.0, 90.0, 1.0, 0.05, 0.20, barrier_type="down_and_in", option_type="put"
    )
    assert result["price"] >= 0
...
def test_barrier_up_and_out_put():
    result = barrier_option_price(
        100.0, 100.0, 120.0, 1.0, 0.05, 0.20, barrier_type="up_and_out", option_type="put"
    )
    assert result["price"] >= 0
...
def test_barrier_down_and_out_call():
    result = barrier_option_price(
        100.0, 100.0, 80.0, 1.0, 0.05, 0.20, barrier_type="down_and_out", option_type="call"
    )
    assert result["price"] >= 0
...
def test_barrier_with_rebate():
    result = barrier_option_price(
        100.0, 100.0, 80.0, 1.0, 0.05, 0.20, barrier_type="down_and_out", rebate=5.0
    )
    assert result["price"] >= 0
...
def test_barrier_t_zero():
    result = barrier_option_price(
        100.0, 90.0, 80.0, 0.0, 0.05, 0.20, barrier_type="down_and_out", option_type="call"
    )
    assert result["price"] == pytest.approx(10.0, abs=0.01)
...
def test_barrier_invalid_barrier_type():
    with pytest.raises(ValueError):
        barrier_option_price(
            100.0, 100.0, 80.0, 1.0, 0.05, 0.20, barrier_type="sideways_and_confused"
        )


def test_lookback_fixed_call():
    result = lookback_option_price(
        100.0, 95.0, 1.0, 0.05, 0.20, strike_type="fixed", option_type="call"
    )
    assert result["price"] > 0
...
def test_lookback_fixed_put():
    result = lookback_option_price(
        100.0, 105.0, 1.0, 0.05, 0.20, strike_type="fixed", option_type="put"
    )
    assert result["price"] > 0
...
def test_lookback_mc():
    result = lookback_option_price(
        100.0, 100.0, 1.0, 0.05, 0.20, method="monte_carlo", n_simulations=2000
    )
    assert result["price"] > 0
...
def test_mc_european_sigma_zero():
    ...
    result = mc_european_price(110.0, 100.0, 1.0, 0.05, 0.0, n_simulations=500, seed=0)
    assert result["price"] > 0


def test_mc_asian_geometric():
    result = mc_asian_price(
        100.0, 100.0, 1.0, 0.05, 0.20, averaging="geometric", n_simulations=2000, seed=0
    )
    assert result["price"] > 0
...
def test_mc_asian_floating():
    result = mc_asian_price(
        100.0, 100.0, 1.0, 0.05, 0.20, strike_type="floating", n_simulations=2000, seed=0
    )
    assert result["price"] > 0
...
def test_mc_asian_put():
    result = mc_asian_price(
        100.0, 100.0, 1.0, 0.05, 0.20, option_type="put", n_simulations=2000, seed=0
    )
    assert result["price"] > 0
