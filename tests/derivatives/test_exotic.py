...
from oprim.derivatives.exotic import (
    _barrier_cf,
    _bs_vanilla,
...
def test_barrier_down_in_out_parity_call():
    s_val, k_val, h_val, t_val, r, sigma = 100.0, 100.0, 80.0, 1.0, 0.05, 0.20
...
def test_barrier_up_in_out_parity_call():
    s_val, k_val, h_val, t_val, r, sigma = 100.0, 100.0, 120.0, 1.0, 0.05, 0.20
    vanilla = _bs(s_val, k_val, t_val, r, sigma, "call")
    out = barrier_option_price(
        s_val,
        strike=k_val,
...
def test_barrier_very_high_upout_approaches_vanilla():
    s_val, k_val, h_val, t_val, r, sigma = 100.0, 100.0, 999.0, 1.0, 0.05, 0.20
    vanilla = _bs(s_val, k_val, t_val, r, sigma, "call")
    result = barrier_option_price(
        s_val,
        strike=k_val,
...
def test_barrier_cf_t0_knocked_out_down():
    ...
    result = _barrier_cf(70.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_out", "call", 5.0)
    assert result == pytest.approx(5.0)


def test_barrier_cf_t0_not_knocked_out():
    # S > H for down_and_out: return intrinsic
    result = _barrier_cf(110.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_out", "call", 5.0)
    assert result == pytest.approx(10.0)


def test_barrier_cf_t0_knocked_in_down():
    # S <= H for down_and_in: return intrinsic
    result = _barrier_cf(75.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_in", "call", 5.0)
...
def test_barrier_cf_t0_not_knocked_in():
    ...
    result = _barrier_cf(110.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_in", "call", 5.0)
    assert result == pytest.approx(5.0)


def test_barrier_cf_t0_up_knocked_out():
    # S >= H for up_and_out → rebate
    result = _barrier_cf(130.0, 100.0, 120.0, 0.0, 0.05, 0.20, 0.0, "up_and_out", "call", 3.0)
    assert result == pytest.approx(3.0)


def test_barrier_cf_t0_up_not_knocked_out():
    # S < H for up_and_out → intrinsic
    result = _barrier_cf(110.0, 100.0, 120.0, 0.0, 0.05, 0.20, 0.0, "up_and_out", "call", 3.0)
    assert result == pytest.approx(10.0)


def test_barrier_cf_t0_up_knocked_in():
    # S >= H for up_and_in → intrinsic
    result = _barrier_cf(130.0, 100.0, 120.0, 0.0, 0.05, 0.20, 0.0, "up_and_in", "call", 3.0)
...
def test_barrier_cf_t0_put():
    result = _barrier_cf(90.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_out", "put", 0.0)
...
def test_barrier_cf_t0_down_out_breached():
    ...
    result = _barrier_cf(75.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_out", "call", 5.0)
    assert result == 5.0
...
def test_barrier_cf_t0_down_in_breached():
    ...
    result = _barrier_cf(75.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_in", "put", 0.0)
    assert abs(result - 25.0) < 1e-10


def test_barrier_cf_t0_up_in_breached():
    """S >= H, up_and_in → returns intrinsic (line 60)."""
    result = _barrier_cf(130.0, 100.0, 120.0, 0.0, 0.05, 0.20, 0.0, "up_and_in", "call", 0.0)
    assert abs(result - 30.0) < 1e-10


def test_barrier_cf_t0_in_not_breached():
    """Not breached, in barrier → returns rebate (line 61)."""
    result = _barrier_cf(100.0, 100.0, 80.0, 0.0, 0.05, 0.20, 0.0, "down_and_in", "call", 7.0)
    assert result == 7.0
...
def test_barrier_zero_vol_out_breached_returns_rebate():
    ...
    result = _barrier_cf(90.0, 100.0, 95.0, 1.0, 0.05, 0.0, 0.0, "down_and_out", "call", 5.0)
...
def test_barrier_zero_vol_out_not_breached_put():
    ...
    result = _barrier_cf(100.0, 90.0, 80.0, 1.0, 0.05, 0.0, 0.0, "down_and_out", "put", 0.0)
    import math

    fwd = 100.0 * math.exp(0.05)
    expected = max(90.0 - fwd, 0.0) * math.exp(-0.05)
    assert abs(result - expected) < 1e-6


def test_barrier_zero_vol_in_breached_call():
    """In barrier, breached, call, zero vol (line 207)."""
    result = _barrier_cf(90.0, 100.0, 95.0, 1.0, 0.05, 0.0, 0.0, "down_and_in", "call", 0.0)
    # Breached → call payoff
    import math

    fwd = 90.0 * math.exp(0.05)
    expected = max(fwd - 100.0, 0.0) * math.exp(-0.05)
    assert abs(result - expected) < 1e-6


def test_barrier_zero_vol_in_breached_put():
    """In barrier, breached, put, zero vol (line 208)."""
    result = _barrier_cf(90.0, 100.0, 95.0, 1.0, 0.05, 0.0, 0.0, "down_and_in", "put", 0.0)
    import math

    fwd = 90.0 * math.exp(0.05)
    expected = max(100.0 - fwd, 0.0) * math.exp(-0.05)
    assert abs(result - expected) < 1e-6


def test_barrier_zero_vol_in_not_breached_rebate():
    """In barrier, not breached, zero vol → rebate * disc (line 209)."""
    result = _barrier_cf(100.0, 90.0, 80.0, 1.0, 0.05, 0.0, 0.0, "down_and_in", "call", 3.0)
...
def test_barrier_cf_down_out_already_knocked():
    ...
    result = _barrier_cf(75.0, 100.0, 80.0, 1.0, 0.05, 0.20, 0.0, "down_and_out", "call", 2.0)
    assert result == 2.0
...
def test_barrier_cf_rebate_nonzero():
    ...
    result = _barrier_cf(100.0, 100.0, 80.0, 1.0, 0.05, 0.20, 0.0, "down_and_out", "call", 5.0)
    assert result >= 0
