...
def test_pattern_detection_hammer():
    ...
    ohlcv = OHLCVInput(open=[10.0], high=[10.02], low=[9.0], close=[9.8], volume=[1000])
...
def test_pattern_detection_engulfing():
    # Bullish engulfing
    ohlcv = OHLCVInput(
        open=[10, 9.5], high=[10.1, 10.6], low=[9.4, 9.4], close=[9.5, 10.5], volume=[1000, 1200]
    )
    res = pattern_detection(ohlcv=ohlcv)
...
def test_pattern_detection_none():
    ...
    res = pattern_detection(ohlcv=ohlcv)
    # Not a hammer (upper shadow 0.5, body 0.5, lower shadow 1.0. Lower shadow
    # is only 2x body, but upper shadow is 1x body > 0.2x)
    assert len(res) == 0
