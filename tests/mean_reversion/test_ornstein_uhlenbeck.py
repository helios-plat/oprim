...
class TestOUFit:
    def test_ou_fit_returns_four_keys(self):
        ...
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 500)
        result = ornstein_uhlenbeck_fit(x_val, dt=1 / 252)
    ...
    def test_ou_fit_synthetic_known_params(self):
        ...
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 2000, seed=7)
        result = ornstein_uhlenbeck_fit(x_val, dt=1 / 252)
    ...
    def test_ou_fit_random_walk_returns_low_theta(self):
        ...
        x_val = np.cumsum(rng.standard_normal(500))
        result = ornstein_uhlenbeck_fit(x_val)
    ...
    def test_ou_fit_half_life_formula(self):
        ...
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 1000)
        result = ornstein_uhlenbeck_fit(x_val, dt=1 / 252)
        if not math.isnan(result["theta"]):
    ...
    def test_ou_fit_dt_parameter(self):
        ...
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 500)
        r1 = ornstein_uhlenbeck_fit(x_val, dt=1 / 252)
        r2 = ornstein_uhlenbeck_fit(x_val, dt=1.0)
    ...
    def test_ou_fit_pandas_series_in(self):
        ...
        x_val = simulate_ou(1.0, 0.0, 0.5, 1 / 252, 100)
        s = pd.Series(x_val)
    ...
    def test_ou_fit_smith_2010_example(self):
        ...
        theta = -log(rho)/dt where rho = corr(x_val_t, x_val_{t-1}), rtol=0.05.
        """
        np.random.default_rng(123)
        x_val = simulate_ou(1.5, 0.5, 0.3, 1.0, 200, seed=123)
        result = ornstein_uhlenbeck_fit(x_val, dt=1.0)
        # Manually compute rho
        rho = float(np.corrcoef(x_val[:-1], x_val[1:])[0, 1])
        if rho > 0:
...
class TestOUHalfLife:
    def test_ou_half_life_regression_basic(self):
        ...
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 500)
        hl = ornstein_uhlenbeck_half_life(x_val, method="regression")
        assert hl > 0

    def test_ou_half_life_mle_basic(self):
        """OU sim, mle half_life > 0."""
        x_val = simulate_ou(2.0, 0.0, 0.5, 1 / 252, 500)
        hl = ornstein_uhlenbeck_half_life(x_val, method="mle")
        assert hl > 0
    ...
    def test_ou_half_life_invalid_method_raises(self):
        ...
        x_val = simulate_ou(1.0, 0.0, 0.5, 1.0, 50)
        with pytest.raises(ValueError, match="Unknown method"):
            ornstein_uhlenbeck_half_life(x_val, method="bogus")
    ...
    def test_ou_half_life_pandas_input(self):
        """pd.Series input is accepted."""
        x_val = simulate_ou(1.0, 0.0, 0.5, 1.0, 100)
        s = pd.Series(x_val)
    ...
    def test_ou_half_life_mle_pandas(self):
        ...
        x_val = simulate_ou(1.0, 0.0, 0.3, 1.0, 200)
        s = pd.Series(x_val)
    ...
    def test_ou_half_life_regression_formula(self):
        ...
        x_val = simulate_ou(3.0, 0.0, 0.2, 1.0, 1000, seed=99)
        hl = ornstein_uhlenbeck_half_life(x_val, method="regression")
        dx = x_val[1:] - x_val[:-1]
        x_lag = x_val[:-1]
...
class TestOUFitExtraGaps:
    def test_ou_fit_theta_nonpositive_returns_nan(self):
        ...
        np.random.default_rng(0)
        # Random walk: x_val_t = x_val_{t-1} + eps -> rho close to 1 but theta very small
        # For theta <= 0, we force a strictly increasing monotone series
        x_val = np.linspace(0, 100, 100)  # perfect trend, rho = 1.0, theta = -log(1)/dt = 0
        result = ornstein_uhlenbeck_fit(x_val, dt=1.0)
...
class TestOUHalfLifeExtraGaps:
    def test_ou_half_life_constant_series_var_zero(self):
        ...
        x_val = np.full(50, 3.14)
        result = ornstein_uhlenbeck_half_life(x_val, method="regression")
    ...
    def test_ou_half_life_mle_negative_rho_returns_inf(self):
        ...
        x_val = np.tile([-1.0, 1.0], 50)
        result = ornstein_uhlenbeck_half_life(x_val, method="mle")
    ...
    def test_ou_half_life_mle_perfect_trend_theta_zero(self):
        ...
        x_val = np.arange(100, dtype=float)
        result = ornstein_uhlenbeck_half_life(x_val, method="mle")
        # rho may be 1.0 exactly (inf) or 0.9999... (very large finite) depending
        # on float arithmetic
